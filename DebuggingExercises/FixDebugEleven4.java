...
public class FixDebugEleven4
{
   ...
   public static void buildArray()
   {
     ...
     for(int x = 0; x < boatArray.length; ++x)
     {
        boatType = getBoat();
        if(boatType =='r')
            //'new' keyword for instantiating objects into an array
 	     boatArray[x] = new FixDebugRowboat();
        else
          boatArray[x] = new FixDebugOceanLiner();
     }
   }
   //Returning a char value (remember void doesn't return any values to method from which it was called
   ...
   {
       //need to be incrementing, not decrementing
      for(int x = 0; x < boatArray.length; ++x)
